...
#[test]
fn emit_elfobj_arm64_with_u64_param_succeeds() {
    // Regression test: ParamLoad { param_idx: 0, ty: U64 } previously caused
    // "unsupported instruction" error in the AArch64 object emitter.
    let root = repo_root();
    let fixture = root
        .join("tests")
        .join("must_pass")
        .join("arm64_param_u64.kr");
    let output_path = unique_temp_output_path("emit-elfobj-arm64-param", "o");
    fs::remove_file(&output_path).ok();
...
mmio_reg UART0.DR = 0x04 : u32 rw;

@ctx(thread, boot)
fn entry() {
  mmio_read<u32>(UART0 + 0x00, first);
  mmio_read<u32>(UART0 + 0x00, second);
  mmio_write<u32>(UART0 + 0x04, second);
}
"#,
    );
...
@ctx(thread, boot)
fn zero_path() {
  mmio_write<u32>(UART0 + 0x04, 0);
}

@ctx(thread, boot)
fn nonzero_path() {
  mmio_write<u32>(UART0 + 0x04, 1);
}

@ctx(thread, boot)
fn entry() {
  mmio_read<u32>(UART0 + 0x00, status);
  branch_if_zero(other, zero_path, nonzero_path);
}
"#,
    );
    ...
    assert_eq!(
        stderr.lines().collect::<Vec<_>>(),
        vec![
            "canonical-exec: function 'entry' contains unsupported branch_if_zero(other, zero_path, nonzero_path): branch test slot 'other' does not match the captured executable slot 'status' in this function"
        ]
    );
...
fn entry() {
  mmio_read<u32>(UART0 + 0x00, status);
  branch_if_eq(status, 0x100000000, ready_path, fallback_path);
}
"#,
    );
    let output_path =
    ...
    assert_eq!(
        stderr.lines().collect::<Vec<_>>(),
        vec![
            "canonical-exec: function 'entry' contains unsupported branch_if_eq(status, 0x100000000, ready_path, fallback_path): branch comparison literal '0x100000000' does not fit u32"
        ]
    );
...
fn entry() {
  mmio_read<u32>(UART0 + 0x00, status);
  branch_if_mask_nonzero(status, 0x100000000, ready_path, fallback_path);
}
"#,
    );
...
fn entry() {
  ...
  stack_cell<u32>(saved_control);
  mmio_read<u32>(UART0 + 0x00, value);
