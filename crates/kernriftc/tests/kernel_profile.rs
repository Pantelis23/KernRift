...
fn kernel_profile_denies_block_in_critical_region() {
    ...
    let stderr = String::from_utf8(assert.get_output().stderr.clone()).expect("stderr utf8");
    assert!(
        stderr.contains("critical-region: CRITICAL_BLOCK_BOUNDARY:"),
        "expected CRITICAL_BLOCK_BOUNDARY violation, got:\n{}",
        stderr
    );
}
...
#[test]
fn kernel_profile_does_not_need_irq_specific_raw_mmio_rule_by_default() {
    let root = repo_root();
    let fixture = root
        .join("tests")
        .join("must_pass")
    ...
    assert!(
        stderr.contains("policy: KERNEL_RAW_MMIO_FORBID: raw_mmio is not allowed"),
        "expected kernel profile to deny irq raw mmio via global raw-mmio rule, got:\n{}",
        stderr
    );
    assert!(
        !stderr.contains("KERNEL_IRQ_RAW_MMIO_FORBID"),
