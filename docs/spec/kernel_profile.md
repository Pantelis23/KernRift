...
- policy rule metadata is cataloged centrally (`code`, `family`, `sort_rank`, `requires_v2`)
  - definitions also carry `default_enabled_in_profile_kernel`, `diagnostic_template_id`, canonical profile `materialization_actions`, canonical `enablement_probes`, and rule `trigger_kind` / `artifact_dependencies`
