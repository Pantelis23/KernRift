...
echo "[1/13] canonical check smoke (file, exit 1)"
...
echo "[2/13] canonical check JSON smoke (file, exit 1)"
CHECK_JSON_OUT="$TMP_DIR/check.json.out"
set +e
run_kernriftc check --canonical --format json "$WORK_FIXTURE" >"$CHECK_JSON_OUT"
status=$?
set -e
if [[ "$status" -ne 1 ]]; then
  echo "expected check --canonical --format json exit code 1, got $status" >&2
  exit 1
fi
assert_contains '"schema_version": "kernrift_canonical_findings_v2"' "$CHECK_JSON_OUT"
assert_contains "\"file\": \"$WORK_FIXTURE\"" "$CHECK_JSON_OUT"
assert_contains '"canonical_findings": 5' "$CHECK_JSON_OUT"
assert_unchanged_fixture

echo "[3/13] canonical dry-run smoke (file)"
...
echo "[4/13] canonical stdout smoke (file)"
...
echo "[5/13] canonical diff smoke (file)"
...
echo "[6/13] canonical edit-plan JSON smoke (file)"
...
echo "[7/13] canonical edit-plan text smoke (file)"
...
echo "[8/13] canonical check smoke (stdin, exit 1)"
...
echo "[9/13] canonical dry-run smoke (stdin)"
...
echo "[10/13] canonical stdout smoke (stdin)"
...
echo "[11/13] canonical diff smoke (stdin)"
...
echo "[12/13] canonical edit-plan text smoke (stdin)"
...
echo "[13/13] canonical edit-plan JSON smoke (stdin)"
