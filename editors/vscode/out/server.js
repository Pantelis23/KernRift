...
connection.onCompletion((params) => {
  ...
  KEYWORDS.forEach((kw) => {
    items.push({ label: kw, kind: import_node.CompletionItemKind.Keyword });
  });
  TYPES.forEach((t) => {
    items.push({ label: t, kind: import_node.CompletionItemKind.TypeParameter });
  });
  ...
  while ((match = enumRegex.exec(text)) !== null) {
    items.push({
      label: match[1],
      kind: import_node.CompletionItemKind.Enum
    });
  }
  return items;
});
...
${BUILTINS[word].doc}`
  ...
  if (KEYWORDS.includes(word)) {
    ...
    if (kwDocs[word]) {
      return { contents: { kind: import_node.MarkupKind.Markdown, value: `**${word}** \u2014 ${kwDocs[word]}` } };
    }
  }
  if (TYPES.includes(word)) {
    const sizes = {
      "uint8": "8-bit unsigned (1 byte)",
    ...
    return { contents: { kind: import_node.MarkupKind.Markdown, value: `**${word}** \u2014 ${sizes[word] || "Type alias"}` } };
  }
  const fnMatch = text.match(new RegExp(`fn\\s+${word}\\s*\\(([^)]*)\\)(?:\\s*->\\s*(\\w+))?`));
  if (fnMatch) {
    const params2 = fnMatch[1];
...
connection.onDefinition((params) => {
  ...
  if (match) {
    ...
    return import_node.Location.create(params.textDocument.uri, import_node.Range.create(pos, pos));
  }
